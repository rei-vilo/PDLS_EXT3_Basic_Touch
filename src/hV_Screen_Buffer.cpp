...
bool hV_Screen_Buffer::getTouch(uint16_t & x, uint16_t & y, uint16_t & z, uint16_t & t)
{
    ...
    s_getRawTouch(x0, y0, z0, t0);
    z = z0;
    t = t0;

    if (z > v_touchTrim)
    {
        x0 = checkRange(x0, v_touchXmin, v_touchXmax);
        y0 = checkRange(y0, v_touchYmin, v_touchYmax);

        switch (v_orientation)
        {
            case 0: // ok

                x = map(x0, v_touchXmin, v_touchXmax, 0, v_screenSizeH);
                y = map(y0, v_touchYmin, v_touchYmax, 0, v_screenSizeV);
                break;

            case 1: // ok

                x = map(y0, v_touchYmin, v_touchYmax, 0, v_screenSizeV);
                y = map(x0, v_touchXmin, v_touchXmax, v_screenSizeH, 0);
                break;

            case 2: // ok

                x = map(x0, v_touchXmin, v_touchXmax, v_screenSizeH, 0);
                y = map(y0, v_touchYmin, v_touchYmax, v_screenSizeV, 0);
                break;

            case 3: // ok

                x = map(y0, v_touchYmin, v_touchYmax, v_screenSizeV, 0);
                y = map(x0, v_touchXmin, v_touchXmax, 0, v_screenSizeH);
                break;
        }
        return true;
    }
...
void hV_Screen_Buffer::s_getRawTouch(uint16_t & x0, uint16_t & y0, uint16_t & z0, uint16_t & t0)
{
    x0 = 0;
    y0 = 0;
    z0 = 0;
