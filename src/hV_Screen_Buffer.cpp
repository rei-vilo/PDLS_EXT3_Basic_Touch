    ...
    if (z > _touchTrim)
    {
        ...
        switch (_orientation)
        {
            case 0: // ok
                ...
                x = map(x0, _touchXmin, _touchXmax, 0, _screenSizeH);
                y = map(y0, _touchYmin, _touchYmax, 0, _screenSizeV);
                break;

            case 1: // ok

                x = map(y0, _touchYmin, _touchYmax, 0, _screenSizeV);
                y = map(x0, _touchXmin, _touchXmax, _screenSizeH, 0);
                break;

            case 2: // ok

                x = map(x0, _touchXmin, _touchXmax, _screenSizeH, 0);
                y = map(y0, _touchYmin, _touchYmax, _screenSizeV, 0);
                break;

            case 3: // ok

                x = map(y0, _touchYmin, _touchYmax, _screenSizeV, 0);
                y = map(x0, _touchXmin, _touchXmax, 0, _screenSizeH);
                break;
        }
        return true;
    }
